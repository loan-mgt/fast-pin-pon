...
const getAssignmentContext = `-- name: GetAssignmentContext :one
SELECT
    ia.id,
    ia.intervention_id,
    ia.unit_id,
...
type GetAssignmentContextRow struct {
...
func (q *Queries) GetAssignmentContext(ctx context.Context, id pgtype.UUID) (GetAssignmentContextRow, error) {
	row := q.db.QueryRow(ctx, getAssignmentContext, id)
	var i GetAssignmentContextRow
	err := row.Scan(
		&i.ID,
		&i.InterventionID,
		&i.UnitID,
...
const getInterventionEventContext = `-- name: GetInterventionEventContext :one
SELECT
    i.id,
    e.event_type_code,
    e.severity,
...
type GetInterventionEventContextRow struct {
	ID            pgtype.UUID        `json:"id"`
	EventTypeCode string             `json:"event_type_code"`
	Severity      int32              `json:"severity"`
...
func (q *Queries) GetInterventionEventContext(ctx context.Context, id pgtype.UUID) (GetInterventionEventContextRow, error) {
	row := q.db.QueryRow(ctx, getInterventionEventContext, id)
	var i GetInterventionEventContextRow
	err := row.Scan(
		&i.ID,
		&i.EventTypeCode,
		&i.Severity,
