...
def fetch_simulator_tick(sim_url: str, tick_count: int) -> Optional[List[Dict]]:
    try:
        response = requests.get(f"{sim_url.rstrip('/')}/tick", params={"count": tick_count}, timeout=5)
...
def main() -> None:
    ...
    poll_interval = float(get_env("SIM_POLL_INTERVAL", "1"))
    ...
    try:
        while True:
            ...
            # Send one microbit in rotation
            if microbit_ids and now - last_send >= send_interval:
                microbit_id = microbit_ids[rotation_idx % len(microbit_ids)]
                rotation_idx = (rotation_idx + 1) % len(microbit_ids)
